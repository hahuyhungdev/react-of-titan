...
3. Open `src/app/router.tsx` and ensure `/help` is listed in the layout wrapper arrays (`authPaths` or `mainPaths`) to wrap it in the correct layout. For example, add `"/help"` to `mainPaths`.
